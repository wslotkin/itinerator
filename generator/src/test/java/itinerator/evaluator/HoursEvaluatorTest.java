...
public class HoursEvaluatorTest {
    ...
    private HoursEvaluator hoursEvaluator;

    @Before
    public void before() {
        hoursEvaluator = new HoursEvaluator(INVALID_HOURS_PENALTY);
    }

    @Test
    public void whenItineraryIsEmptyShouldReturnZero() {
        double result = hoursEvaluator.evaluate(new Itinerary(emptyList()));
    ...
    public void shouldReturnNumberOfInvalidEventsMultipliedByPenalty() {
        double result = hoursEvaluator.evaluate(new Itinerary(newArrayList(createEventWithStart(7),
